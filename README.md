...
This program implements a simple interactive double-ended queue (deque) using an array and allows the user to manipulate it in real time through short commands. The deque is represented by a structure containing a fixed-size array along with three key variables: front, rear, and count, which track the positions of elements and how many items are currently stored. The program supports inserting elements at both the front and rear, as well as deleting elements from both ends, which directly reflects the defining characteristic of a deque—bidirectional access. Internally, it uses a circular array technique (modulo operation) so that when either pointer reaches the end of the array, it wraps around to the beginning, ensuring efficient use of space. The interactive loop in main continuously accepts user commands to perform operations such as adding or removing elements and displaying the current contents. This behavior demonstrates how a deque generalizes both stacks and queues: inserting and deleting at the rear mimics stack (LIFO) behavior, while inserting at the rear and deleting from the front mimics queue (FIFO) behavior. By allowing all four operations, the program clearly shows how a deque combines and extends both data structures into a more flexible system.
